...
Each main entity has a primary key. `passenger_id`, `flight_id`, `booking_id`, and `airport_id` provide identifiers that are independent of display names or other attributes that may change. A booking receives its own identifier because it represents an individual event rather than just a relationship between a passenger and a flight. The `flight_routes` relation is different because it represents a many-to-many relationship. Its composite primary key, `(flight_id, airport_id)`, guarantees that a particular flight-airport association cannot be stored more than once.
...
This decision also reflects the expected write pattern. Bookings are high-volume transactional records, while flight deletion should be relatively rare. Therefore, it is more important to protect booking history than to optimize for the deletion of a flight.
